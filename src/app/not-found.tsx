...
export default function NotFound() {
    ...
    return (
        <div className="min-h-screen bg-[var(--bg-page)] flex flex-col items-center justify-center p-4 text-center animate-fade-in relative overflow-hidden text-[var(--text-primary)]">
            ...
            <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-[var(--color-brand-blue)]/5 rounded-full blur-3xl -z-10 animate-float" style={{ animationDelay: '0s' }}></div>
            <div className="absolute bottom-1/4 right-1/4 w-64 h-64 bg-[var(--color-brand-red)]/5 rounded-full blur-3xl -z-10 animate-float" style={{ animationDelay: '1s' }}></div>

            <div className="app-card p-12 max-w-lg w-full flex flex-col items-center">
                <div className="w-24 h-24 bg-[var(--bg-surface-highlight)] rounded-full flex items-center justify-center mb-6">
                    <FileQuestion className="w-12 h-12 text-[var(--color-brand-yellow)]" />
                </div>

                <h1 className="app-title text-8xl font-medium text-[var(--text-primary)] mb-2 tracking-tighter">404</h1>
            ...
            <div className="mt-12 text-sm text-[var(--text-tertiary)] flex justify-center gap-6">
                <span className="flex items-center gap-1.5 before:w-2 before:h-2 before:rounded-full before:bg-[var(--color-brand-green)]">
                    System Operational
                </span>
                <span>StudyMate.io v2.4.0</span>
            </div>
        </div>
    );
}
